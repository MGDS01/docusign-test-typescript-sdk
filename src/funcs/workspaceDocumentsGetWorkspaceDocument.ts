 ...
 */
export function workspaceDocumentsGetWorkspaceDocument(
  client: DocusignTypescriptTest2Core,
  request: operations.GetWorkspaceDocumentRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.GetWorkspaceDocumentResponse,
    | errors.ErrorDetails
    | APIError
    | SDKValidationError
    | UnexpectedClientError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetWorkspaceDocumentRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc(
    "/v1/accounts/{accountId}/workspaces/{workspaceId}/documents/{documentId}",
  )(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GetWorkspaceDocumentResponse$inboundSchema),
