 ...
 */
export function workspacesCreateWorkspaceEnvelope(
  client: DocusignTypescriptTest2Core,
  request: operations.CreateWorkspaceEnvelopeRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.CreateWorkspaceEnvelopeResponse,
    | errors.ErrorDetails
    | APIError
    | SDKValidationError
    | UnexpectedClientError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateWorkspaceEnvelopeRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.WorkspaceEnvelopeForCreate, {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.CreateWorkspaceEnvelopeResponse$inboundSchema),
