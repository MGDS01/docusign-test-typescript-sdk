 ...
 */
export function workspaceUsersUpdateWorkspaceUser(
  client: DocusignTypescriptTest2Core,
  request: operations.UpdateWorkspaceUserRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.UpdateWorkspaceUserResponse,
    | errors.ErrorDetails
    | APIError
    | SDKValidationError
    | UnexpectedClientError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateWorkspaceUserRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.WorkspaceUserForUpdate, {
    explode: true,
  });
  ...
  const path = pathToFunc(
    "/v1/accounts/{accountId}/workspaces/{workspaceId}/users/{userId}",
  )(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.UpdateWorkspaceUserResponse$inboundSchema),
