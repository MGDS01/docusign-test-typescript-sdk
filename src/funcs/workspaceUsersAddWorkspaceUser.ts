 ...
 */
export function workspaceUsersAddWorkspaceUser(
  client: DocusignTypescriptTest2Core,
  request: operations.AddWorkspaceUserRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.CreateWorkspaceUserResponse,
    | errors.ErrorDetails
    | APIError
    | SDKValidationError
    | UnexpectedClientError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.AddWorkspaceUserRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.WorkspaceUserForCreate, {
    explode: true,
  });
  ...
  const path = pathToFunc(
    "/v1/accounts/{accountId}/workspaces/{workspaceId}/users",
  )(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.CreateWorkspaceUserResponse$inboundSchema),
